...
**Two database roles, on purpose, and neither is a superuser.** Flyway migrates as `nine_owner`. The service runs as `nine_app`: not a superuser, not the owner of any table, and holding no `UPDATE` or `DELETE` grant on ledger tables at all. Superuser and owner both bypass row-level security, so the runtime role must be neither or every policy is decoration, and the owner must not be a superuser or the `FORCE` on those policies is decoration in turn: measured on `accounts`, a superuser owner reads every row with no tenant bound and `nine_owner` reads none, and its way out (`SET row_security = off`) raises rather than returning them. That holds for the four tenant tables; `api_keys` and the two reconciliation tables carry policies that allow a read with no tenant on purpose, so the owner is subject to those policies rather than above them. The immutability tests prove both layers: `nine_app` gets `permission denied` before the trigger is consulted; a superuser, which needs no grant at all, is stopped by the trigger anyway.
...
# Once per database, as a superuser, before Flyway: creates nine_owner and hands
# it the database it is connected to, so the database goes in the URL and the
# script refuses the cluster default rather than handing over the wrong one. The
# compose stack does this for a database it creates, so this line is for one that
# is already there. It does not move tables another role already owns, which is
# BI12.5's second half.
